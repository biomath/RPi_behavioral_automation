...
if __name__ == "__main__":
    ...
    if is_debug is "y":
        ...
        while True:
            protocol = raw_input("Choose protocol: Switch test(1), Introduction (2), Shaping (3), Go/No-go (4): ")
            session_id = raw_input("Session/animal ID: ")
            ...
            try:
                if protocol is "1":
                    ...
                    prompt = raw_input("Press enter to start. ")
                    session.introduction()
                elif protocol is "3":
                    prompt = raw_input("Press enter to start. ")
                    session.shaping()
                    ...
                    prompt = raw_input("Press enter to start. ")
                    session.go_nogo(go_sound=go_sound, nogo_sound=nogo_sound, probability=prob)
                GPIO.cleanup()
            except KeyboardInterrupt:
                GPIO.cleanup()
                exit_or_rerun = raw_input("Exit (1) or Rerun (2)? ")
